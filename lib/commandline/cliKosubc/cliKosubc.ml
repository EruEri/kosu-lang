...
module Cli = struct
 ...
 let cmd_term run = 
  let combine f_allow_generic_in_variadic no_std
    is_target_asm output files = 
    run @@ {f_allow_generic_in_variadic; no_std; is_target_asm; output; files}
  in
  Term.( 
    const combine 
    $ f_allow_generic_in_variadic_term
    $ no_std_term
    $ target_asm_term
    $ output_term
    $ files_term
  ...
  let kosubc_man = [
    ...
    `P 
      "The Kosu bytecode compiler $(b,kosu.bc) compiles kosu files into bytecodes which \ 
