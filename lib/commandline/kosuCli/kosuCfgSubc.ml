...
let module_term =
  Arg.(
    ...
    & info ~docv:"module"
        ~doc:
          "look for the $(b,function) into $(docv). If $(docv) start with a \
...
let cfg_man =
  [
    ...
    `P
      "kosu-cfg allows you to visual control flow graphs (cfg) and variable \
       inference graph";
    `P
      "kosu-cfg relies heavily on the $(b,dot) language and the $(b,graphviz) \
...
let cfg_main cmd =
  ...
  let function_decl_list =
    filtered_function_in_module
    |> List.filter_map (function TNFunction fn -> Some fn | _ -> None)
  in
