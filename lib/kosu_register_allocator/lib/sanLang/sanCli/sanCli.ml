...
module CmdError = struct
  type t = No_function of string

  exception CdmError of t

  let string_of_cmd_erorr = function
    | No_function s ->
        Printf.sprintf "No function \"%s\" was defined" s
end
...
module Cfg_Command = struct
  ...
  let function_name_term =
    Arg.(
      value
      & opt (some string) None
      & info ~doc:"Generate graph for a the $(opt) function"
          ~docv:"function_name" [ "f"; "function" ]
    )

  let cfg_type_term =
    Arg.(
      value
      & opt (enum cfg_type_enum) Detail
      & info
  ...
  let file_term =
    let info = Arg.info [] ~docv:"FILE" in
  ...
  let cfg_man =
    [
      ...
      `P
        "san-cfg allows you to visual control flow graphs (cfg) and variable \
         inference graph";
      `P
        "san-cfg relies heavily on the $(b,dot) language and the $(b,graphviz) \
         library";
  ...
  let cfg_name ~extension ~ctype fn_name =
    Printf.sprintf "%s.%s.%s" fn_name
      (cfg_type_enum |> lswap |> List.assoc ctype)
      extension

  let dot_infered_name = infered_name ~extension:"dot"
  let dot_cfg_name = cfg_name ~extension:"dot"

  let target_file file_type format fn_name =
    match format with
    | None -> (
    ...
    | true ->
        ...
        let transform =
          if colored then
            ...
            SanCfg.SanCfgPprint.export_infer_graph_of_cfg
        in
  ...
  let export_from_san_function cmd
    ...
    | None -> (
        let outchan_name =
          target_file (`Cfg cmd.cfg_type) cmd.dot san_function.fn_name
        in
        let () =
        ...
        | true ->
            let infered_ouchan =
              target_file (`Infered cmd.colored) cmd.dot san_function.fn_name
            in
    ...
    | Some export_format -> (
        ...
        let _ =
          Sys.command
            (Printf.sprintf "dot -T%s -o %s %s" export_format cfg_outname
               filename
            )
        in
        match cmd.variable_infer with
        ...
        | true ->
            let infered_ouchan =
              target_file (`Infered cmd.colored) cmd.dot san_function.fn_name
            in
            let tmp_infered_filename, tmp_infered_out =
              Filename.open_temp_file "infered" ".dot"
            in
            let () =
              write_infered ~infered:cmd.variable_infer ~colored:cmd.colored
                ~oc:tmp_infered_out san_function
            in
    ...
    let typed_san_module =
      ...
      |> Option.map (fun s ->
             ...
             |> List.filter (fun node ->
                    ...
                    match node with
                    | TyDeclaration { fn_name; _ } | TyExternal { fn_name } ->
                        fn_name = s
                )
         )
      |> Option.value ~default:typed_san_module
    in
    let filtered_san_module =
      typed_san_module
      |> List.filter_map
           (let open SanTyped.SanTyAst in
            function TyDeclaration fn -> Some fn | TyExternal _ -> None
           )
    in

    let () =
      filtered_san_module
      |> List.iter (fun (san_function : SanTyped.SanTyAst.ty_san_function) ->
...
module Compile_Cmd = struct
  ...
  let other_file_term =
    Arg.(
      ...
      & info ~doc:"Pass files to $(b,cc)(1)" ~docv:"FILES" []
    )
  ...
  let cmd_term run =
    ...
    in
    Term.(
      const combine $ target_asm_term $ output_term $ file_term
      $ other_file_term
    )
  ...
  let compile_man =
    [
      `S Manpage.s_description;
      `P
        "san-compile allows you to compile san program into executable or \
         assembly files";
      `P "san-compile use the C compilateur to generate the executable";
      `S Manpage.s_see_also;
      `Noblank;
      `P "$(b,cc)(1)";
      `Noblank;
  ...
  let compile_main cmd =
    ...
    let module Codegen =
      SanBackend.Aarch64.Codegen.Make
        (SanBackend.Aarch64.Implementation.MacOSAarch64AsmSpec) in
    match asm with
    | true ->
        let outfile =
          if outfile = default_outfile then
    ...
    | false ->
        let () =
          Codegen.compile ~outname:outfile other_files ty_san_module ()
        in
        ()
...
let san_man =
  [
    `S Manpage.s_description;
    `P "San is the compiler of a minimalist 3 address code language";
    `P "San exists to more easily test the register allocator of Kosu";
    `P "San produce the executable by invoking the C compiler";
    `S Manpage.s_see_also;
    `P "$(b,cc)(1)";
