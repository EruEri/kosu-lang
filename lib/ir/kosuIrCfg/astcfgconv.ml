...
let rec of_tac_statements ~tag_map ~start_label ~end_labels ~ending
  ...
  | stmt :: q as _stmts -> (
      ...
      | STIf
          ...
          in
          let if_blocks =
            of_tac_body ~tag_map ~end_labels:[ exit_label ] if_tac_body
          in
          let else_blocks =
