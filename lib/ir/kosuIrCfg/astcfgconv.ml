  ...
  | stmt :: q as _stmts -> (
      ...
      | STWhile
          ...
          let blocks_continuation =
            of_tac_statements ~start_label:exit_label ~end_labels ~ending
              ~cfg_statements:[] rprogram (q, return)
          in
      ...
      | STSwitch
          ...
          let enum_decl =
            ...
            in
            KosuIrTyped.Asttyhelper.Renum.instanciate_enum_decl generics
              enum_decl
          in
          ...
          | sw_case :: sw_switch_remains -> (
              ...
              | variant :: else_variants ->
                  ...
                  let basic_blocks, next_block_label =
                    sw_case_without_first_match :: sw_switch_remains
                    |> List.mapi Util.couple
                    |> List.fold_left
                         (fun (acc, block_next_label) (index, sw_switch) ->
                           let is_last = branch_count - 1 = index in
                           let blocks, next_label =
                             _of_switch_case ~wildcard_label ~enum_decl
                               ~tag_atom ~start_label:block_next_label ~is_last
                               rprogram sw_switch
                           in
                  ...
                  let wildcard_basic_block =
                    ...
                    |> Option.map (fun body ->
                           let block_setup =
                             of_tac_statements ~start_label:next_block_label
                               ~end_labels:[ Option.get wildcard_label ]
                               ~ending:None ~cfg_statements:[] rprogram
                               ([], None)
                           in
                           let block_body =
      ...
      | STSwitchTmp
          ...
          let wildcard_basic_block =
            ...
            |> Option.map (fun body ->
                   let block_setup =
                     of_tac_statements ~start_label:final_label
                       ~end_labels:[ Option.get tmp_wildcard_label ]
                       ~ending:None ~cfg_statements:[] rprogram ([], None)
                   in
                   let block_body =
          ...
          let continuation =
            of_tac_statements ~start_label ~end_labels:local_endlabel
              ~ending:local_ending ~cfg_statements rprogram
              (tmp_statemenets_for_case @ [ first_cmp_stmt ], None)
          in

          let blocks_continuation =
            of_tac_statements ~start_label:tmp_sw_exit_label ~end_labels ~ending
              ~cfg_statements:[] rprogram (q, return)
          in
...
and of_switch_case rprogram (tac_switch : tac_switch_tmp) =
  ...
  let body_block =
    of_tac_body
      ~end_labels:[ tac_switch.tmp_sw_exit_label ]
      rprogram tac_switch.tmp_switch_tac_body
  in
...
and _of_switch_case ~is_last ~wildcard_label ~enum_decl ~tag_atom ~start_label
  ...
  | variants ->
      ...
      let other_variants, next_block_label =
        variants |> List.mapi Util.couple
        |> List.fold_left
             (fun (acc, block_start_label) (index, variant) ->
      ...
      let body_block =
        of_tac_body
          ~end_labels:[ sw_switch.sw_exit_label ]
          rprogram sw_switch.switch_tac_body
      in
