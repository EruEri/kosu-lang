...
type module_resolver = 
  ModuleResoler of string location list

module TyLoc = struct
  type kosu_loctype_polymorphic = 
  | PolymorphicVar of string location
  type kosu_loctype = 
  ...
  | TyLocOpaque of { module_resolver : module_resolver; name : string location }
  | TyLocOrdered
  | TyLocStringLit
  | TyLocChar
  | TyLocBool
...
| PAs of {
  pas_pattern: kosu_pattern location;
...
and kosu_unary_op = UMinus of kosu_expression location | UNot of kosu_expression location
and kosu_block = {
