...
module Make (Spec : X86_64AsmSpec.X86_64AsmSpecification) = struct
  ...
  let translate_tac_rvalue ?(is_deref = None) ~litterals
    ...
    | RVArrayAccess {array_expr; index_expr} -> 
      ...
      let r10, index_instructions = translate_tac_expression ~litterals ~target_dst:(`Register r10) rprogram fd index_expr in
      let r9, array_instructions = translate_tac_expression ~litterals ~target_dst:(`Register r9) rprogram fd array_expr in
      ...
      let mov_instructions = 
        ...
        | true -> 
          ...
          in
          offset_reg_instruction::mult_offset_instruction::
          offset_instruction::shift_instruction::[]
        | false -> 
          ...
          let load_index_instruction = 
            ...
            | true -> 
              Instruction (
                Mov {
                  size = IntSize (Option.get @@ data_size_of_int64 @@ rvalue_size);
                  destination = r9;
      ...
      in
      let copy_instructions = copy_result ~where ~register:(Register.r9) ~rval_rktype rprogram in
      index_instructions @ array_instructions @ mov_instructions @ copy_instructions
    | RVAdress id ->
        let pointee_type =
