  ...
  let translate_tac_rvalue ?(is_deref = None) ~litterals
    ...
    | RVBuiltinCall { fn_name; parameters } -> (
        ...
        | Array_len ->
          ...
          in
          let rr9 = Register.r9 in
          let mov_instr = Instruction ( Mov { size = iq; source = `ILitteral array_len; destination = `Register rr9} ) in
          let copy_instructions =
            copy_result ~where ~register:rr9 ~rval_rktype rprogram
          in
        ...
        | Array_ptr -> 
          let tte = List.hd parameters in
          let rr9 = Register.r9 in
          let _, instructions = translate_tac_expression ~litterals ~target_dst:(`Register rr9) rprogram fd tte in
          let copy_instructions =
            copy_result ~where ~register:rr9 ~rval_rktype rprogram
          in
