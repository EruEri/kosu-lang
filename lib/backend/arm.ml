...
| B of {
  condition_code: condition_code option;
  label: label
}
...
module IStack = struct
  ...
  type stacks = (info * t)
  let empty: stacks = [], []
  let push_instruction (inst: instruction) (list, stack):stacks = list, inst::stack
  let current_offset (stacks: stacks) = stacks |> fst |> List.fold_left (fun acc (_, _, off) -> acc + off) 0

  let padding_needed rktype rprogram (stacks) = 
    let aligmentof = (KosuIrTyped.Asttyped.Sizeof.alignmentof rprogram rktype) |> Int64.to_int in
...
let instructions_of_function generics program function_decl = 
  ...
  let combined = generics |> List.combine function_decl.generics |> List.to_seq |> Hashtbl.of_seq in
  function_decl.rparameters |> List.map (fun (_field, rktype) ->
