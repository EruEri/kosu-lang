...
module ValidateExternalFunction = struct
  ...
  let does_contains_not_c_compatible_type program current_module
    ...
    |> List.filter_map (fun kt ->
           ...
           with Ast.Error.Ast_error e -> Error.Ast_Error e |> Option.some)
    |> function
    | [] -> Ok ()
    | t :: _ -> t |> Result.error
  ...
  let is_valid_struct_decl program (current_module_name : string)
    ...
    | Some field ->
        let located_field =
          ...
          |> fun (f, _) -> f
        in
        Error.SDuplicated_field { field = located_field; struct_decl }
        |> Error.struct_error |> Result.error
end
...
module ValidateEnum = struct
  ...
  let is_valid_enum_decl program (current_module_name : string)
    ...
    | Some vari ->
        ...
        in
        Error.EDuplicated_variant_name { variant = located_variant; enum_decl }
        |> Error.enum_error |> Result.error
end
  ...
  let check_parameters_duplicate (function_decl : function_decl) =
    ...
    | t :: _ ->
        let duplicate =
          ...
          |> List.rev |> List.hd |> fst
        in
        Error.Duplicated_parameters
          { duplicatated_field = duplicate; function_decl }
        |> Error.function_error |> Result.error
...
module ValidateModule = struct
  let check_duplicate_function { path; _module } =
    ...
    | [] -> Ok ()
    | t :: _ ->
        Error.Duplicate_function_declaration { path; functions = t }
  ...
  let check_duplicate_operator { path; _module } =
    ...
    | [] -> Ok ()
    | t :: _ ->
        Error.Duplicate_operator_declaration { path; operators = t }
        |> Error.module_error |> Result.error
  ...
  let check_duplicate_type { path; _module } =
    ...
    | [] -> Ok ()
    | t :: _ ->
        Error.Duplicate_type_declaration { path; types = t }
        |> Error.module_error |> Result.error
  ...
  let check_duplicate_const_name { path; _module } =
    ...
    | [] -> Ok ()
    | t :: _ ->
        Error.Duplicate_const_declaration { path; consts = t }
        |> Error.module_error |> Result.error
...
module Validate_Program = struct
  let check_main_in_program (program : program) =
    ...
    in
    if count < 2 then Ok ()
    else Error.Too_many_Main filtered_function_decl |> Result.error
end
