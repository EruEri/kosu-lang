...
struct
  ...
  module VError = struct
    ...
    type syscall_error =
      ...
      | Syscall_Too_much_parameters of {
  ...
  module Help = struct
          ...
          | Ok (Ast.Type_Decl.Decl_Struct struct_decl) ->
              let new_struct =
                Asthelper.Struct.bind_struct_decl
                  (parametrics_type |> List.map Position.value)
      ...
      then
        ...
        || struct_decl.fields
           |> List.exists (fun (_, kt) ->
                  match kt.v with
                  | TType_Identifier _
                    when Struct.is_type_generic kt.v struct_decl ->
                      false
                  | TType_Identifier
                      ...
                      let t_decl =
                        match
                          Asthelper.Program.find_type_decl_from_ktype
                            ~ktype_def_path ~ktype_name ~current_module program
                        with
                        | Error e ->
                            e |> Ast.Error.ast_error |> raise
                        | Ok type_decl ->
                  ...
                  |> List.exists (fun kt ->
                         ...
                         | TParametric_identifier
                             ...
                             let t_decl =
                               match
                                 Asthelper.Program.find_type_decl_from_ktype
                                   ~ktype_def_path ~ktype_name ~current_module
                                   program
                               with
                               | Error e ->
                                   e |> Ast.Error.ast_error |> raise
    ...
    let program_remove_implicit_type_path program =
      program
      |> List.map
           (fun { filename; module_path = { path; _module = Mod _module } } ->
             let new_module =
               _module
               |> List.map (fun node ->
  ...
  module ValidateSyscall = struct
    ...
    let does_contains_too_much_parameters (syscall_decl : syscall_decl) =
      ...
      if length > 5 then
        VError.Syscall_Too_much_parameters
  ...
  module ValidateStruct = struct
    ...
    let is_valid_struct_decl program (current_module_name : string)
      ...
      >>= (fun () ->
            if Help.is_cyclic_struct current_module_name program struct_decl
            then
              VError.SCyclic_Declaration struct_decl |> VError.struct_error
              |> Result.error
            else
              Ok ()
          )
      ...
      | Some field ->
          let located_field =
            struct_decl.fields
            |> List.find_all (fun (fie, _) -> fie.v = field)
            |> List.rev |> List.hd
            |> fun (f, _) -> f
          in
  ...
  module ValidateEnum = struct
    let is_all_type_exist current_module program (enum_decl : enum_decl) =
      ...
      |> List.flatten
      |> List.find_map (fun kt ->
             if Asthelper.Enum.is_ktype_generic_level_zero kt.v enum_decl then
               None
             else
               match
  ...
  module ValidateFunction_Decl = struct
    ...
    let check_parameters_duplicate (function_decl : function_decl) =
      ...
      | t :: _ ->
          let duplicate =
            function_decl.parameters
            |> List.find_all (fun (field, _) -> field.v = t)
            |> List.rev |> List.hd |> fst
          in
    ...
    let is_all_type_exist current_module program function_decl =
      ...
      |> List.find_map (fun kt ->
             ...
             then
               ...
               match
                 Help.is_ktype_exist_from_ktype function_decl.generics
                   current_module program kt.v
               with
               | Ok () ->
                   None
    ...
    let check_main_sig function_decl =
      if not (is_main_function function_decl) then
        Ok ()
      else if is_valid_main_sig function_decl then
        Ok ()
      else
    ...
    let check_function program current_module function_decl =
      ...
      >>= fun () ->
      check_main_sig function_decl
      >>= fun () ->
      check_unit_parameters function_decl
      >>= fun () ->
      check_parameters_duplicate function_decl
  ...
  module ValidateOperator_Decl = struct
      ...
      | Unary u ->
          let expected =
            Asthelper.ParserOperator.expected_unary_return_type
              ...
              u.op.v
          in
          if u.return_type.v === expected then
            Ok ()
          else
            Op_wrong_return_type_error
              {
                op = operator_decl |> Asthelper.ParserOperator.operator;
                expected;
                found = u.return_type.v;
              }
            |> VError.operator_error |> Result.error
      | Binary b ->
          let expected =
            Asthelper.ParserOperator.expected_binary_return_type
              ...
              b.op.v
          in
          if b.return_type.v === expected then
            Ok ()
          else
            Op_wrong_return_type_error
              {
    ...
    let is_valid_operator_decl current_module program operator_decl =
      let ( >>= ) = Result.bind in
      is_all_type_exist current_module program operator_decl
      >>= (fun () -> check_parameters operator_decl)
  ...
  module ValidateModule = struct
    ...
    let check_duplicate_operator { path; _module } =
      ...
      |> Util.ListHelper.duplicated (fun lop rop ->
             ...
             | Binary l, Binary r ->
                 ...
                 && l.fields |> fst |> snd |> Position.value
                    === (r.fields |> fst |> snd |> Position.value)
                 && l.fields |> snd |> snd |> Position.value
                    === (r.fields |> snd |> snd |> Position.value)
                 && l.return_type |> Position.value
                    === (r.return_type |> Position.value)
             | Unary l, Unary r ->
                 l.op |> Position.value = (r.op |> Position.value)
                 && l.field |> snd |> Position.value
                    === (r.field |> snd |> Position.value)
                 && l.return_type |> Position.value
                    === (r.return_type |> Position.value)
             | _, _ ->
                 false
    ...
    let check_main_signature { path; _module } =
      ...
      let function_decls =
        ...
        |> List.filter (fun decl ->
               decl |> Ast.Function_Decl.calling_name |> Position.value
               |> ( = ) "main"
           )
      in
      function_decls
      |> List.fold_left
           (fun acc value ->
             ...
             | Ast.Function_Decl.Decl_Syscall syscall_decl ->
                 Main_no_kosu_function (`syscall_decl syscall_decl)
                 |> VError.module_error |> Result.error
    ...
    let check_validate_module _module =
      ...
      >>= fun () ->
      check_duplicate_function _module
      >>= fun () ->
      check_duplicate_operator _module
      >>= fun () ->
  ...
  module Validate_Program = struct
    let check_main_in_program (program : program) =
      let filtered_function_decl =
        ...
        |> List.map (fun { path; _module } ->
               ( path,
                 _module |> Asthelper.Module.retrieve_functions_decl
                 |> List.filter (fun fn ->
                        fn |> Ast.Function_Decl.calling_name |> Position.value
                        = "main"
                    )
               )
           )
        |> List.filter (fun (_path, fn_decls) ->
               fn_decls |> List.length |> ( <= ) 1
           )
      in
      let count =
      ...
      if count < 2 then
        ...
        VError.Too_many_Main filtered_function_decl |> Result.error
  end
