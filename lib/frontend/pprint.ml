...
let register_kosu_error () =  
  ...
  | Lexer.Forbidden_char (position, char) -> begin
    ...
    Printf.sprintf "\n%s: Forbidden character : %c" s char |> Option.some 
  end
  | Lexer.Unexpected_escaped_char (position, lexeme) -> begin 
    let s = position |> string_of_position_error in
    Printf.sprintf "\n%s: Unexpected Escaped character: %s" s lexeme |> Option.some
  end
  | Lexer.Invalid_keyword_for_build_in_function (position, id) -> begin 
    let s = position |> string_of_position_error in
    Printf.sprintf "\n%s: Invalid Keyword For Builtin Function: %s" s id |> Option.some
  end
  | Lexer.Invalid_litteral_for_build_in_function (position, litteral) -> begin 
    let s = position |> string_of_position_error in
    Printf.sprintf "\n%s: Invalid Litteral For Builtin Function: %c" s litteral |> Option.some
  end
  | Lexer.Not_finished_built_in_function position -> position |> string_of_position_error |> Printf.sprintf "\n%s: Builtin function not finished" |> Option.some
  | Lexer.Unclosed_comment position -> position |> string_of_position_error |> Printf.sprintf "\n%s: Comments not terminated" |> Option.some
  | Lexer.Unclosed_string position -> position |> string_of_position_error |> Printf.sprintf "\n%s: String litteral not terminated" |> Option.some 
  | _ -> None ) 