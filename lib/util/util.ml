...
module PkgConfig = struct
  ...
  let create ?(include_path = StringSet.empty) ?(libs_path = StringSet.empty) ?(linked_libs = StringSet.empty) others 
    = {
      include_path;
      libs_path;
      linked_libs;
  ...
  let parse_single pkg_config option = 
    ...
    else match String.sub option 0 2 with
  ...
  let linker_flags_format pkg_config = 
      let union = StringSet.union in
      pkg_config.linked_libs
      |> union pkg_config.libs_path
      |> StringSet.elements
      |> String.concat " "
      |> Printf.sprintf "%s"
