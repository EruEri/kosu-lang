...
struct 
  ...
  let add_node node ienv = {
    ienv with modules = node::ienv.modules
  }


  let string_of_parameters kts = 
    let open Position in
    kts |> List.map (fun kt -> Pprint.string_of_ktype kt.v) |> String.concat ", "
    

  let string_of_node = let open Position in function
  ...
  | NFunction function_decl ->
    ...
    (function_decl.parameters |> List.map (fun (_, kt) -> Pprint.string_of_ktype kt.v) |> String.concat ", "
    )
    (Pprint.string_of_ktype function_decl.return_type.v)
  | NOperator operator_decl ->
    let operator = Position.value @@ Asthelper.ParserOperator.backticked_operator operator_decl in
    let return_type = Position.value @@ Asthelper.ParserOperator.return_ktype operator_decl in
    let parameters = Asthelper.ParserOperator.parameters operator_decl in
    Printf.sprintf "op ( %s ) : (%s) : %s = <operator fun>" 
      operator
    ...
    | Ok (Some inode) -> begin 
      try
        ...
        | IStatement stmt -> 
          ...
          repl env ()
        | IModule_Node node -> 
          let () = match FrontEnd.Astvalidation.validate_module_node (module_path_of_ienv ienv) "" node with
          | Ok () -> ()
          | Error e -> raise @@ FrontEnd.Astvalidation.VError.Validation_error ("", e)
          in
